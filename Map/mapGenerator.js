...
export class MapGenerator {
    constructor(scene, seed = 'JergBuilder_Default', mapSize = 32) {
        this.scene = scene;
        this.seed = seed;
        
        // Medium Map Configuration
    ...
    initMaterials() {
        ...
        const grassTexture = textureLoader.load(
            ...
            undefined,
            (err) => console.warn('Grass texture load fallback used.', err)
        );
        ...
        const waterTexture = textureLoader.load(
            ...
            undefined,
            (err) => console.warn('Water texture load fallback used.', err)
        );
    ...
    // Dynamic registration method for external Mods
    ...
    generate(seedOverride = null) {
        ...
        for (let x = -halfSize; x < halfSize; x++) {
            for (let z = -halfSize; z < halfSize; z++) {
                // 0.08 scaling gives smooth, medium-scale hills across the 32x32 area
                const nx = x * 0.08;
                const nz = z * 0.08;
                const rawHeight = this.noise.get2D(nx + 100, nz + 100);
                const maxTerrainHeight = Math.floor(rawHeight * 8) + 1;
                ...
                // Fill water up to baseline height
